...
- `src/EdgeBrake/EdgeBrake.tsx`：DOM 界面、按住/松手输入层、角色属性牌、力度槽、HUD、收藏与结算。
...
- `src/EdgeBrake/assets/characters/`：52 个 GLB、52 张 PNG 缩略图和 `ASSETS.json`，构建不依赖仓库外路径。
- `src/EdgeBrake/i18n/index.ts`：中英文文案和语言检测。
- `src/EdgeBrake/utils/sounds.ts`：Web Audio 合成音效。
- `src/EdgeBrake/EdgeBrake.less`：HUD、蓄力控制、属性牌、结果层、收藏卡和响应式视觉系统。
...
- 蓄力与物理：80 ms 为最短有效输入，1600 ms 充满；力度为 `0.12 + 0.88 × normalized^1.35`。初速度为 `(335 + 365 × power) × speedFactor`，角色速度 2.6–5.0 映射为 0.86–1.14。每帧使用匀减速更新，减速度为 `clamp(126 × surfaceFactor × (75 / weight)^0.22, 92, 166)`；晴、雪、雾、风雪系数分别为 1.00、1.07、0.96、1.03。速度低于 3 px/s 并稳定 140 ms 后结算。
...
- 角色动作：人形/机甲驱动 `rig_legL/rig_legR/rig_armL/rig_armR`，四足、鸟、蛙与悬浮角色使用独立动作组。蓄力随力度连续下蹲和前后错步，82% 以上加入轻颤；松手前 620 ms 加强蹬地步幅和前倾，随后过渡为滑行平衡。失败冻结后隐藏原模型，按角色色板生成 14 个方块碎片完成 1250 ms 慢动作散落。
...
- 场景：`IcePlatform` 用不同长度和厚度的 BoxGeometry 形成方块浮岛跑道；天气按 6 关循环；远景从熊、牛、猪、狐狸、幽灵、僵尸、狼人、木乃伊、骷髅、战斗机甲和牛头怪中随机抽取巨大局部剪影。角色使用实时阴影，跑道不生成额外矩形假阴影。
...
- 新增或同步角色：把同源 GLB/PNG 与清单放入 `assets/characters/`；调整体重、速度、价格、缩放、朝向或动作分组：修改 `characters.ts`。
- 修改角色动作、浮岛、天气、灯光、镜头、粒子或远景巨物：修改 `components/EdgeBrakeScene.tsx`，并同步 `doc/visual.md`。
- 修改 HUD、蓄力控件、属性牌、结果页、颜色、排版与动效：修改 `EdgeBrake.tsx`、`EdgeBrake.less` 和 `doc/visual.md`。
