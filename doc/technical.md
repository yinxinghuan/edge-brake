...
- 游戏阶段：首局使用 `cover → ready → playing`；停车后进入不限时 `result`，只有“下一位角色”或“本关重试”按钮才调用 `advanceResult()`，不存在自动推进计时器。通过后自动换角并进入下一关的 520 ms `ready`；过早停车则保留分数、金币、角色与关卡，先进入 `awaiting`，玩家再次点击才发车。越过冰崖进入 `falling → gameover`，失败结算的“再来一次”同样只进入 `awaiting`。
