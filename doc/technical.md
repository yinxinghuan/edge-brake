...
- 游戏阶段：首局使用 `cover → ready → playing`；成功后进入 `result` 并衔接下一关，越过冰崖后进入 `falling → gameover`。结算层的“再来一次”只进入 `awaiting` 近景准备态，玩家再次点击屏幕或角色后才进入 `ready`，不会自动开始第二局。
- 物理与评分：角色以 160 px/s 起步，起点 x=40、随机崖口 x=1820–1940（按角色前缘计算的有效滑行距离约 1731–1851 px）；未制动时按跑道进度使用 65 / 160 / 95 px/s² 三段加速度，第 1 关最高速度 390 px/s、逐关增加并封顶 480 px/s，因此完整滑行约 5.0–6.2 秒；单击后以 `320 × 当前角色抓地系数` px/s² 持续减速，速度低于 3 px/s 并稳定 110 ms 后结算；依据角色前缘到冰崖的 0–8、9–22、23–52 和 53+ px 四档计算得分、连击和金币。
- 角色与动作：`useGLTF` 加载共享素材库 GLB 并 clone 当前角色；人物 GLB 的正面是本地 +Z，因此使用 `headingYaw = π/2` 对齐跑道 +X，狐狸、青蛙和棕熊的正面本来就是本地 +X，因此保持 `headingYaw = 0`。人物网格按空间位置重建肩/髋枢轴，并优先识别身体外侧的手部，避免低位手掌被误挂到腿部；动物低位网格按空间象限重建四足枢轴。滑行动作使用约 0.72–0.95 秒的交替蹬冰，人物左右腿以约 ±1.08 rad 交替摆动，双臂以相反相位前后摆并轮流向外展开；急停时四肢大幅外撑、压低重心、后仰并加入高频回稳颤动。失败阶段不再旋转完整 GLB：原模型冻结 140 ms 后隐藏，`CharacterShatter` 按角色主色生成 14 个 BoxGeometry 碎块，以确定性速度、重力和错相旋转完成 1250 ms 慢动作散落；减少动态效果时保留分解但降低旋转与下坠幅度。
...
- 3D 映射与镜头：逻辑层使用屏幕空间数值，场景层通过 `screenToWorld()` 映射到世界 X 轴。cover、`awaiting`、每关 520 ms 的 ready 以及上一关 result 后段都把相机回收到角色前方约 35° 的 zoom 64 起点近景，使面部、胸口与跑道朝向同时可读；进入 playing 的同一帧记录近景相机快照并开放刹车，再用确定性 ease-out 在 1050 ms 内拉到 zoom 6.8 的全跑道机位，完整看到起点与崖口后持续停留到 2050 ms。之后 `FollowCamera` 随危险进度连续推进、降低并绕到人物正前侧，急停时立即提高跟随权重并保证 zoom 不低于 54；falling 阶段推至 zoom 74，观察点在 1250 ms 内缓慢下移追随碎片。长跑道机位同步扩大天气雾距，避免远端冰面被雾裁掉；zoom 乘以实际渲染尺寸相对 390×700 的比例，避免页面整体缩放与 R3F ResizeObserver 造成窄屏二次缩放。
...
- 输入：首屏透明语义按钮和封面空白区域的 `onPointerDown` 启动游戏，幽灵手指不接收输入；gameover 结算按钮在独立 `onClick` 中执行复位并进入 `awaiting`，其 pointerdown 阻止冒泡；`retryUnlockAtRef` 再提供 500 ms 发车锁，只有锁解除后的下一次游戏区域 `onPointerDown` 或 Space / Enter 才发车。playing 阶段游戏区域一次 `onPointerDown` 或一次 Space / Enter 锁定本回合制动，ready 阶段输入被忽略，重复输入不会取消或重新触发。
