...
- `src/EdgeBrake/EdgeBrake.tsx`：DOM 界面、按住/松手输入层、三行角色信息、力度槽、HUD、收藏与结算。
...
- `src/EdgeBrake/EdgeBrake.less`：HUD、蓄力控制、三行角色信息、结果层、收藏卡和响应式视觉系统。
...
- 蓄力与物理：80 ms 为最短有效输入，1600 ms 充满；力度为 `0.12 + 0.88 × normalized^1.35`。初速度为 `(335 + 365 × power) × speedFactor`，角色速度 2.6–5.0 映射为 0.86–1.14。每帧使用匀减速更新，减速度为 `clamp(126 × surfaceFactor × (75 / weight)^0.22, 92, 166)`；晴、雪、雾、风雪系数分别为 1.00、1.07、0.96、1.03。`velocity / deceleration <= 1.25` 或 `remainingToCliff / velocity <= 0.9` 时把 `isAutoBraking` 置为 true，只切换动作、雪屑和声音，不修改减速度。速度低于 3 px/s 并稳定 140 ms 后结算。
...
- 角色动作：人形/机甲驱动 `rig_legL/rig_legR/rig_armL/rig_armR`，四足、鸟、蛙与悬浮角色使用独立动作组。蓄力随力度连续下蹲和前后错步，82% 以上加入轻颤；松手前 620 ms 加强蹬地步幅和前倾，随后过渡为滑行平衡。自动刹车恢复旧版后仰与四肢外撑动作，按动作组分别处理双臂、翼和四足，并让 `SnowSpray` 在脚后持续循环。失败冻结后隐藏原模型，按角色色板生成 14 个方块碎片完成 1250 ms 慢动作散落。
...
- 修改 HUD、蓄力控件、三行角色信息、结果页、颜色、排版与动效：修改 `EdgeBrake.tsx`、`EdgeBrake.less` 和 `doc/visual.md`。
