...
- 物理与评分：未刹车时按 92 px/s² 加速，按住时按 310 px/s² 减速；依据企鹅前缘到冰崖的 0–8、9–22、23–52 和 53+ px 四档计算得分与连击。
- 3D 映射：逻辑层继续使用稳定的屏幕空间数值，场景层通过 `screenToWorld()` 映射到世界 X 轴；正交相机保持赛道水平，避免透视影响判距。
- 渲染：场景资产由低段数基础几何拼装，全部使用 flat-shaded 高粗糙度材质；主光、填充光、轮廓光、雾和接触阴影统一体积感。
- 输入：游戏区域 `onPointerDown` 开始制动，抬起或取消解除；Space / Enter 支持键盘按住；失焦与页面隐藏会主动解除制动。
